...
OUTPUT_FIXED_HORIZON_ATTRITION = TABLES_DIR / "dislocation_fixed_horizon_attrition.csv"
...
FIG_FIXED_HORIZON_HEADLINE_VS_SKIP1 = FIGURES_DIR / "dislocation_fixed_horizon_headline_vs_skip1_ci.png"
...
def add_past_only_zscores(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    if "business_gap_days" in out.columns:
        out["residual_gap_bucket"] = pd.cut(
            ...
            bins=[0, 1, 3, 5, np.inf],
            labels=["1bd", "2-3bd", "4-5bd", "gt5bd"],
            include_lowest=True,
            right=True,
        ).astype("string")
        gap_group_cols = group_cols + ["residual_gap_bucket"]
        out = _past_expanding_stats_by_group_date(
            out,
            value_col="m4_residual_return",
            group_cols=gap_group_cols,
            out_prefix="m4_issuer_maturity_gap",
            min_obs=MIN_GROUP_HISTORY_OBS,
        )
    ...
    if "m4_issuer_maturity_gap_z" in out.columns:
        out["m4_abs_z_gap_conditioned"] = out[
...
def add_fair_value_prediction_intervals(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    for level in PREDICTION_INTERVAL_LEVELS:
        ...
        q = _empirical_conformal_quantile(
...
def classify_dislocation_events(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    evaluation_mask = out[SPLIT_COL].isin(["validation", "test"])
    out["m4_candidate_flag"] = (
        evaluation_mask
        & out["m4_abs_z_main"].ge(Z_CANDIDATE_THRESHOLD)
    )
    out["m4_severe_flag"] = (
        evaluation_mask
        & out["m4_abs_z_main"].ge(Z_SEVERE_THRESHOLD)
    )
    if "m4_abs_z_gap_conditioned" in out.columns:
        out["m4_candidate_gap_conditioned_flag"] = (
            evaluation_mask
            & out["m4_abs_z_gap_conditioned"].ge(Z_CANDIDATE_THRESHOLD)
        )
        out["m4_candidate_gap_robust_agreement"] = (
            out["m4_candidate_flag"]
            & out["m4_candidate_gap_conditioned_flag"]
        )
...
def _load_convergence_price_path() -> pd.DataFrame:
    ...
    if not TRACE_CONVERGENCE_PRICE_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run trace_bond_data_institutional_cleaner.py first."
        )

    price = pd.read_parquet(TRACE_CONVERGENCE_PRICE_PATH)
    _assert_columns(price, [GROUP_COL, DATE_COL], "convergence price path")
    price[DATE_COL] = pd.to_datetime(price[DATE_COL], errors="coerce")
    price[GROUP_COL] = price[GROUP_COL].astype(str).str.strip()

    price_col = next(
        (
            col
            for col in ["vwap_price", "final_vwap_price"]
    ...
    if price_col is None:
        raise ValueError(
            "The convergence price path contains no VWAP price-level column."
        )

    px = pd.to_numeric(price[price_col], errors="coerce")
    price["_log_vwap_price"] = np.log(px.where(px > 0))
    return (
        price.dropna(subset=[GROUP_COL, DATE_COL, "_log_vwap_price"])
        .sort_values([GROUP_COL, DATE_COL])
        .drop_duplicates([GROUP_COL, DATE_COL], keep="last")
...
def add_future_returns(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped_date = price.groupby(GROUP_COL)[DATE_COL]

    for h in FUTURE_HORIZONS:
        price[f"future_return_{h}obs"] = (
            grouped_price.shift(-h) - price["_log_vwap_price"]
        )
    ...
    out.loc[
        ~out["m4_candidate_flag"].fillna(False),
        "signal_direction",
    ] = 0.0
    ...
    for h in FUTURE_HORIZONS:
        ...
        out[f"future_reversal_return_{h}obs"] = (
            out["signal_direction"] * out[future_col]
        )
        out[f"future_reversal_hit_{h}obs"] = (
            out[f"future_reversal_return_{h}obs"].gt(0)
        )
        out[f"unhedged_signal_pnl_{h}obs"] = (
            out["signal_direction"] * out[future_col]
        )

        out[f"future_reversal_return_skip1_{h}obs"] = (
            out["signal_direction"] * out[skip_col]
        )
        out[f"future_reversal_hit_skip1_{h}obs"] = (
            out[f"future_reversal_return_skip1_{h}obs"].gt(0)
        )
        out[f"unhedged_signal_pnl_skip1_{h}obs"] = (
            out["signal_direction"] * out[skip_col]
        )
...
def _bootstrap_mean_ci_by_cluster(
    g: pd.DataFrame,
    value_col: str,
    cluster_col: str = GROUP_COL,
    n_boot: int = PNL_BOOTSTRAP_N,
    ...
    if len(clusters) < 2:
        ...
        cluster_values = {
            cluster: valid.loc[
                valid[cluster_col].eq(cluster), value_col
        ...
        for _ in range(n_boot):
            ...
            values = np.concatenate([cluster_values[c] for c in sampled])
            boot.append(float(values.mean()))
        method = "cusip_cluster_bootstrap"
...
def build_strategy_pnl_uncertainty(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, g in test.groupby(group_cols, dropna=False):
        ...
        for col in pnl_cols:
            ...
            month_ci = _bootstrap_mean_ci_by_month(
                g=g,
                value_col=col,
            ...
            cusip_ci = _bootstrap_mean_ci_by_cluster(
                g=g,
                value_col=col,
                cluster_col=GROUP_COL,
                n_boot=PNL_BOOTSTRAP_N,
                seed=PNL_BOOTSTRAP_SEED + 101,
            )
            conservative_lower = float(
                min(month_ci["ci95_lower"], cusip_ci["ci95_lower"])
            )
            conservative_upper = float(
                max(month_ci["ci95_upper"], cusip_ci["ci95_upper"])
            )
            ...
            rows.append(
                {
                    ...
                    "ci95_lower_mean_pnl": month_ci["ci95_lower"],
                    "ci95_upper_mean_pnl": month_ci["ci95_upper"],
                    "bootstrap_method": month_ci["bootstrap_method"],
                    "cusip_ci95_lower_mean_pnl": cusip_ci["ci95_lower"],
                    "cusip_ci95_upper_mean_pnl": cusip_ci["ci95_upper"],
                    "cusip_bootstrap_method": cusip_ci["bootstrap_method"],
                    "conservative_ci95_lower_mean_pnl": conservative_lower,
                    "conservative_ci95_upper_mean_pnl": conservative_upper,
                }
            )
...
def build_fixed_horizon_attrition(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    test = signals.loc[
        ...
        & signals["m4_candidate_flag"].fillna(False)
    ].copy()
    if test.empty or future_col not in test.columns:
        return pd.DataFrame()

    test["complete_fixed_horizon"] = test[future_col].notna()
    if "residual_side_simple" not in test.columns:
        test["residual_side_simple"] = np.where(
            test["m4_z_main"].lt(0),
            "negative_residual_potentially_cheap",
    ...
    for keys, g in test.groupby(group_cols, dropna=False):
        row = {
            ...
            "signal_quality_class": keys[1],
            "severity": keys[2],
            "complete_fixed_horizon": bool(keys[3]),
            "n_events": int(len(g)),
            "n_cusips": int(g[GROUP_COL].nunique()),
            "mean_abs_entry_z": float(g["m4_abs_z_main"].mean()),
            "median_abs_entry_z": float(g["m4_abs_z_main"].median()),
        }
        for col in [
            "business_gap_days",
            "log_n_trades",
...
def build_event_driven_strategy_trades(signals: pd.DataFrame) -> pd.DataFrame:
    ...
    price_path = _load_convergence_price_path()
    price_lookup = price_path.set_index(
        [GROUP_COL, DATE_COL]
    )["_log_vwap_price"]
            ...
            for side in strategy_sides:
                ...
                for cusip, g in split_df.groupby(GROUP_COL, sort=False):
                    ...
                    while pos < len(idx_list) - 1:
                        ...
                        entry_date = pd.Timestamp(entry_row[DATE_COL])
                        exit_date = pd.Timestamp(df.at[exit_idx, DATE_COL])
                        entry_key = (str(cusip), entry_date)
                        exit_key = (str(cusip), exit_date)
                        entry_log_price = price_lookup.get(entry_key, np.nan)
                        exit_log_price = price_lookup.get(exit_key, np.nan)

                        gross_bond_payoff = (
                            direction
                            * (float(exit_log_price) - float(entry_log_price))
                            if pd.notna(entry_log_price)
                            and pd.notna(exit_log_price)
                            else np.nan
                        )
                        ...
                        row = {
                            ...
                            "entry_log_vwap_price": entry_log_price,
                            "exit_log_vwap_price": exit_log_price,
                            "holding_calendar_days": (exit_date - entry_date).days,
...
def write_outputs(signals: pd.DataFrame, mean_reversion: pd.DataFrame) -> None:
    ...
    build_fixed_horizon_attrition(signals).to_csv(
        OUTPUT_FIXED_HORIZON_ATTRITION,
        index=False,
    )
...
def plot_fixed_horizon_headline_vs_skip1_ci(
    ...
    if OUTPUT_STRATEGY_PNL_UNCERTAINTY.exists():
        ...
        uncertainty = build_strategy_pnl_uncertainty(signals)
    if uncertainty.empty:
        return

    headline_metric = FIXED_HORIZON_MAIN_PNL_COL
    skip1_metric = (
        f"unhedged_signal_pnl_skip1_"
        f"{FIXED_HORIZON_MAIN_HORIZON_OBS}obs"
        f"_net_{FIXED_HORIZON_MAIN_COST_BPS}bp"
    )

    selected = uncertainty.loc[
        uncertainty["signal_quality_class"].eq(
            EVENT_STRATEGY_HIGH_CONFIDENCE_CLASS
        )
        & uncertainty["severity"].isin(
            [
                "candidate_abs_z_ge_2",
                "severe_abs_z_ge_3",
            ]
        )
        & uncertainty["pnl_metric"].isin(
            [
                headline_metric,
                skip1_metric,
            ]
    ...
    side_labels = {
        "negative_residual_potentially_cheap": "Cheap",
        "positive_residual_potentially_rich": "Rich",
    }
    severity_labels = {
        "candidate_abs_z_ge_2": "Moderate",
        "severe_abs_z_ge_3": "Severe",
    }
    metric_labels = {
        headline_metric: "Headline",
        skip1_metric: "Skip first mark",
    }

    selected["side_label"] = selected[
        "residual_side_simple"
    ].map(side_labels)
    selected["severity_label"] = selected[
        "severity"
    ].map(severity_labels)
    selected["metric_label"] = selected[
        "pnl_metric"
    ].map(metric_labels)
    ...
    selected = selected.dropna(
        subset=[
            ...
            "metric_label",
            "mean_pnl",
            "conservative_ci95_lower_mean_pnl",
            "conservative_ci95_upper_mean_pnl",
        ]
    ).copy()
    ...
    selected["category"] = (
        selected["side_label"]
        + " "
        + selected["severity_label"].str.lower()
    )
    ...
    x_positions = np.arange(len(category_order), dtype=float)
    offsets = {
        "Headline": -0.12,
        "Skip first mark": 0.12,
    }
    markers = {
        "Headline": "o",
        "Skip first mark": "s",
    ...
    for metric_label in ["Headline", "Skip first mark"]:
        metric_data = (
            selected.loc[
                selected["metric_label"].eq(metric_label)
            ]
        ...
        means = metric_data["mean_pnl"].to_numpy(dtype=float) * 100.0
        lower = (
            metric_data["conservative_ci95_lower_mean_pnl"]
            .to_numpy(dtype=float)
            * 100.0
        )
        upper = (
            metric_data["conservative_ci95_upper_mean_pnl"]
            .to_numpy(dtype=float)
            * 100.0
        )
        ...
        error_bars = np.vstack(
            [
                means[valid] - lower[valid],
                upper[valid] - means[valid],
            ]
        )

        plt.errorbar(
            x_positions[valid] + offsets[metric_label],
            means[valid],
            yerr=error_bars,
            fmt=markers[metric_label],
            linestyle="none",
            capsize=4,
            label=metric_label,
        )
    ...
    plt.ylabel("Mean signal-directed payoff (%)")
    plt.xlabel("High-confidence event group")
    plt.title(
        "Fixed-horizon payoff before and after "
        "excluding the first post-signal mark"
    )
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        FIG_FIXED_HORIZON_HEADLINE_VS_SKIP1,
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def write_figures(signals: pd.DataFrame, mean_reversion: pd.DataFrame) -> None:
    ...
    plot_fixed_horizon_headline_vs_skip1_ci(signals)
...
def write_manifest(signals: pd.DataFrame, mean_reversion: pd.DataFrame) -> None:
    manifest = {
        ...
        "event_driven_strategy": {
            ...
            "payoff_types": ["bond_return"],
            "payoff_construction": "direct entry-to-exit VWAP log-price change",
        ...
        "outputs": {
            ...
            "fixed_horizon_attrition": str(OUTPUT_FIXED_HORIZON_ATTRITION),
